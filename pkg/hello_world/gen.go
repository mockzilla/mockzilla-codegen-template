...
func NewRouter(svc ServiceInterface, opts ...RouterOption) chi.Router {
	...
	r.Method("POST", "/hello", http.HandlerFunc(adapter.PostHello))
...
// GeneratePostHelloResponse generates a mock response for POST /hello.
// ctx is an optional replacement context for controlling generated values.
func GeneratePostHelloResponse(ctx map[string]any) (*PostHelloResponseData, error) {
	f, err := GetFactory()
	if err != nil {
		return nil, err
	}
	res, err := f.Response("/hello", "POST", ctx)
	if err != nil {
		return nil, err
	}
	var body PostHelloResponse
...
// GeneratePostHelloResponseBody generates a mock response body for POST /hello.
// Returns just the typed body without headers or status.
// ctx is an optional replacement context for controlling generated values.
func GeneratePostHelloResponseBody(ctx map[string]any) (*PostHelloResponse, error) {
	res, err := GeneratePostHelloResponse(ctx)
	if err != nil {
		return nil, err
	}
	return res.Body, nil
...
// GeneratePostHelloRequest generates a mock request for POST /hello.
// ctx is an optional replacement context for controlling generated values.
func GeneratePostHelloRequest(ctx map[string]any) (schema.GeneratedRequest, error) {
	f, err := GetFactory()
	if err != nil {
...
// NewPostHelloResponseData creates a new PostHelloResponseData with the given body.
func NewPostHelloResponseData(body *PostHelloResponse) *PostHelloResponseData {
	return &PostHelloResponseData{Body: body}
}

// WithHeaders sets custom headers on the response.
func (r *PostHelloResponseData) WithHeaders(h http.Header) *PostHelloResponseData {
	r.Headers = h
	return r
}
